...
)
{
    if (version_data.image_entry == image_entry_format::V0) {
        ...
        const uint8_t ex_count = buffer.read_u8();
...
)
{
    ...
    for (uint32_t i = offsets.images; i < offsets.images_end; i += 4) {
        ...
        const uint8_t* image_data = buffer.at(entry.data_offset);

        const image_container_format format = version_data.image;
        ...
        if (format == image_container_format::ZLIB || format == image_container_format::CHOWIMG) {
            ...
            if (format == image_container_format::ZLIB) {
                const int result = uncompress(temp_buffer.at(0), &out_size, image_data, entry.size);
                if (result != Z_OK) {
                ...
                const int result = chowimg_read(temp_buffer, buffer,
