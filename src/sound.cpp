...
const sound_offsets& get_sound_offsets(const sound_entry_format format)
{
    static constexpr sound_offsets offsets_v0 = {8, 16};
    static constexpr sound_offsets offsets_v1 = {4, 8};
    static constexpr sound_offsets offsets_v2 = {16, 20};
...
const std::string& get_sound_container_extension(const uint32_t sound_container_format_id)
{
...
void extract_audio(
    ...
    const std::string& output_dir_path, const sound_entry_format format
)
{
    ...
    for (uint32_t i = offsets.sounds; i < offsets.sounds_end; i += 4) {
        ...
        const uint32_t container_type = buffer.read_u32();
